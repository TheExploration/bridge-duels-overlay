            ...
            mcApi.getUuid(name).then(uuid => {
                hypixelApi.getPlayer(uuid).then(async (res) => {
                    ...
                    if (player && player.stats["Duels"]) {
                        const wins = player.stats["Duels"]["bridge_duel_wins"] || 0
                        const losses = player.stats["Duels"]["bridge_duel_losses"] || 0
                        const bws = player.stats["Duels"]["best_bridge_winstreak"] || 0
                        const ws = player.stats["Duels"]["current_bridge_winstreak"] || 0
                        const wlr = Math.round((wins / losses) * 100) / 100 || wins
                        const kdr = Math.round((player.stats["Duels"]["bridge_kills"] / player.stats["Duels"]["bridge_deaths"]) * 100) / 100 || player.stats["Duels"]["bridge_kills"]
                        ...
                        winElement.innerHTML = `<span style="color: ${colors["RED"]};">${wins}</span>`
                        lossElement.innerHTML = `<span style="color: ${colors["RED"]};">${losses}</span>`
